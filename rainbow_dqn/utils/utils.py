...
def poll_opponent_dqn(opponents):
    idx = np.random.randint(0, len(opponents))
    return opponents[idx], idx
...
def compute_winning_reward(transition, is_player_one):
    ...
    if transition[4]:
        if transition[6]['winner'] == 0:  # tie
            r += 0
        elif transition[6]['winner'] == 1 and is_player_one:  # you won
            r += 10
        elif transition[6]['winner'] == -1 and not is_player_one:
            r += 10
        else:  # opponent won
            r -= 10
    return r
...
def recompute_rewards(match, username):
    ...
    for transition in transitions:
        ...
        new_transition[3] = compute_winning_reward(transition, is_player_one) + \
            5*compute_reward_closeness_to_puck(transition)
        # new_transition[6]['reward_closeness_to_puck']
...
class Logger:
    ...
    def save_model(self, model, filename):
        ...
        with open(savepath, 'wb') as outp:
            pickle.dump(model, outp, protocol=5)
